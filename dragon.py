...
def scrape_game(game_name, game_id=None, skip_existing=False):
    normalized_names = normalize_name(game_name)
    ...
    for normalized_name in normalized_names:
        ...
        while (d < edate):
            ...
            #logger.log('Game {}, viewers={}, streams={}, peak_viewers={}, for {}'.format(
            ...
            if num_viewers is None or num_streams is None or peak_viewers is None:
                ...
                logger.warning('Failed to fetch={} with normalized={}'.format(game_name, normalized_name))
            else:
...
def normalize_name(game_name):
    ...
    # Gnome sometimes removes the '-' and sometimes keep it.
    # So make up all possibily combinations

    normalized_names = []
    key_letters = '-'
    output_letters = ' -'
    list_norm = list(normalized_name)
    indices = [ i for i, c in enumerate(list_norm) if c in key_letters ]
    for t in product(output_letters, repeat=len(indices)):
        for i, c in zip(indices, t):
            list_norm[i] = c
        normalized_names.append(''.join(list_norm))
